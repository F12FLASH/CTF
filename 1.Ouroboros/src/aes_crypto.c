#include "aes_crypto.h"
#include <string.h>

static const uint8_t sbox[256] = {
    
};

void simple_xor_encrypt(const uint8_t *input, size_t len, const uint8_t *key, uint8_t *output) {
    for (size_t i = 0; i < len; i++) {
        output[i] = input[i] ^ key[i % 32] ^ sbox[i % 256];
    }
}

void aes_encrypt(const uint8_t *plaintext, size_t len, 
                 const uint8_t *key, uint8_t *ciphertext) {
    simple_xor_encrypt(plaintext, len, key, ciphertext);
}

void aes_decrypt(const uint8_t *ciphertext, size_t len,
                 const uint8_t *key, uint8_t *plaintext) {
    simple_xor_encrypt(ciphertext, len, key, plaintext);
}
